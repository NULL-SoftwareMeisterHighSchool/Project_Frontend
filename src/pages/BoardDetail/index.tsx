...
const BoardDetail = () => {
  ...
  const { mutateAsync: putVisibilityMutate } = useMutation(putVisibility, {
    onSuccess: () => {
      alertSuccess("게시물이 공개되었습니다.");
      navigate("/");
    },
    onError: () => {
      alertError("게시물 공개에 실패했습니다.");
    },
  });
      ...
      {blogOpen && (
        <Modal setVal={setBlogOpen}>
          <S.UseTitleContainer>
            ...
            <S.UserSubTitle>삭제한 게시글은 되돌릴 수 없어요.</S.UserSubTitle>
          </S.UseTitleContainer>
          <S.UserBtnContainer>
            <button
              onClick={() => {
        ...
        <S.Post>
          ...
          <S.IconSection>
            <S.Icons>
              ...
              >
                {data.isLiked ? (
                  <Favorite fill={color.critical} width="24px" />
                ) : (
            ...
            <S.Icons>
              {!data.isPrivate ? (
                ...
                <S.UpdateText
                  ...
                  onClick={() => putVisibilityMutate({id, isPrivate})}
                >
                  공개하기
                </S.UpdateText>
              )}
              {data.isAuthor && (
                <S.MyIcon>
                  ...
                  >
                    <Delete fill={color.critical} width="24px" />
                    <S.UpdateText
                      fill={color.critical}
